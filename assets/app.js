...
(function () {
  ...
  function loadChunk(idx, cb) {
    ...
    s.src = "data/chunk_" + String(idx).padStart(2, "0") + ".js?v=38";
    s.onload = function () {
  ...
  function loadSearchPart(i, cb) {
    ...
    var s = document.createElement("script");
    s.src = "data/search_" + String(i).padStart(2, "0") + ".js?v=38";
  ...
  adminModal.innerHTML =
    ...
    '<div class="note-head">管理后台 <button type="button" class="icon-btn" data-close aria-label="关闭">×</button></div>' +
    '<div class="adm-tabs"><button type="button" class="adm-tab on" data-tab="users">👤 用户管理</button>' +
    '<button type="button" class="adm-tab" data-tab="feedback">✉️ 用户反馈<span class="adm-fb-badge" id="adm-fb-badge" hidden></span></button></div>' +
    '<div style="padding: var(--s-4); overflow:auto;"><div id="admin-body">加载中…</div><div id="admin-fb" hidden>加载中…</div></div></div>';
  ...
  function openAdminPanel() {
    ...
    var load = function () {
      fetch("/api/admin/stats").then(function (r) { return r.json(); }).then(function (d) {
        ...
        var html = '<div class="admin-stats">' +
          ...
          "</div>";
        // 管理员永久置顶（#0）；其余按注册序号，可切换升降序（序号跟人走）
        var adminRow = d.users.find(function (u) { return u.is_admin; });
        var normalRows = d.users.filter(function (u) { return !u.is_admin; });
        var sortDir = "desc"; // 默认最新在前
        var sortIcon = function () {
          return sortDir === "desc"
            ? '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 5v14"/><path d="m19 12-7 7-7-7"/></svg>'
        ...
        var renderUserRow = function (u, ui) {
          var initial = ((u.email || "?")[0] || "?").toUpperCase();
          var tag = u.reg_index === 0 ? '<span class="ad-idx0">#0</span>' : "#" + u.reg_index;
          var row = "<tr>" +
            '<td class="ad-user"><span class="ad-avatar c' + ((ui % 3) + 1) + '">' + esc(initial) + '</span>' +
            '<span class="ad-mail-wrap"><span class="ad-mail">' + esc(u.email) + '</span><em>' + tag + "</em></span></td>" +
            ...
            '<td class="ta-c"><span class="ad-pill" title="收藏 ' + (u.fav || 0) + ' · 书签 ' + (u.bookmark || 0) + '">' + (u.notes || 0) + "</span></td>" +
            '<td class="ta-r">';
          if (u.is_admin) {
            row += '<span class="adm-na">—</span>';
          } else {
            row += '<span class="adm-ops">' +
        ...
        var renderTable = function () {
          var sorted = normalRows.slice().sort(function (a, b) {
            return sortDir === "desc" ? b.id - a.id : a.id - b.id;
          });
          var body = "";
          if (adminRow) body += renderUserRow(adminRow, 0);
          sorted.forEach(function (u, i) { body += renderUserRow(u, i + 1); });
          if (!d.users.length) body = "";
          return '<div class="admin-table-wrap"><table class="admin-table"><thead><tr>' +
            '<th>用户</th><th>角色</th><th>注册 IP</th><th>注册地</th>' +
            '<th class="th-sort-head">注册时间 <button type="button" class="th-sort" id="adm-sort-toggle" title="切换排序（新→旧 / 旧→新）">' + sortIcon() + "</button></th>" +
            '<th class="ta-c">标注</th><th class="ta-r">管理</th>' +
            "</tr></thead><tbody>" + body + "</tbody></table>" +
            (d.users.length ? "" : '<div class="ad-empty">还没有注册用户</div>') + "</div>";
        };
        html += renderTable();
        ...
        var bindSort = function () {
          var t = document.getElementById("adm-sort-toggle");
          if (!t) return;
          t.addEventListener("click", function () {
            sortDir = sortDir === "desc" ? "asc" : "desc";
            var wrap = $("admin-body").querySelector(".admin-table-wrap");
            if (wrap) {
              wrap.outerHTML = renderTable();
              bindOps();
              bindSort();
            }
          });
    ...
    var loadFb = function () {
      fetch("/api/admin/feedback").then(function (r) { return r.json(); }).then(function (d) {
        var box = $("admin-fb");
        if (!d.ok) { box.textContent = d.err || "加载失败"; return; }
        var badge = $("adm-fb-badge");
        if (d.open > 0) { badge.textContent = d.open; badge.hidden = false; } else { badge.hidden = true; }
        if (!d.items.length) { box.innerHTML = '<div class="ad-empty">还没有收到用户反馈</div>'; return; }
        var html = '<div class="afb-summary">待处理 <b>' + d.open + "</b> / 共 " + d.total + " 条</div>";
        d.items.forEach(function (f) {
          var who = f.email ? esc(f.email) + (f.user_id ? ' <em>#' + f.user_id + "</em>" : "") : '<span class="afb-guest">游客</span>';
          var catName = f.category === "improve" ? "🛠 改进建议" : "📚 建议收录";
          html += '<div class="afb-item' + (f.merged ? " merged" : "") + '">' +
            '<div class="afb-head"><span class="afb-who">' + who + '</span>' +
            '<span class="afb-cat">' + catName + '</span>' +
            '<span class="afb-date">' + esc(f.created_at.slice(0, 16)) + "</span></div>" +
            '<div class="afb-content">' + esc(f.content) + "</div>" +
            '<div class="afb-foot">' +
            (f.merged
              ? '<span class="afb-merged-tag">已采纳 ✓ ' + esc((f.merged_at || "").slice(0, 16)) + "</span>"
              : '<button type="button" class="afb-merge-btn" data-id="' + f.id + '">Merged · 已采纳</button>') +
            "</div></div>";
        });
        box.innerHTML = html;
        box.querySelectorAll(".afb-merge-btn").forEach(function (btn) {
          btn.addEventListener("click", function () {
            if (!confirm("标记为已采纳？提交者将在其反馈记录中看到「已采纳」。")) return;
            fetch("/api/admin/feedback/" + btn.dataset.id + "/merge", { method: "POST" })
              .then(function (r) { return r.json(); }).then(function (res) {
                if (res.ok) { toast("已标记采纳，用户可见"); loadFb(); }
                else toast(res.err || "操作失败");
              });
          });
        });
      });
    ...
    // Tab 切换
    adminModal.querySelectorAll(".adm-tab").forEach(function (t) {
      t.addEventListener("click", function () {
        adminModal.querySelectorAll(".adm-tab").forEach(function (x) { x.classList.remove("on"); });
        t.classList.add("on");
        var isFb = t.dataset.tab === "feedback";
        $("admin-body").hidden = isFb;
        $("admin-fb").hidden = !isFb;
        if (isFb) loadFb();
      });
    });
    load();
  ...
  /* ================= 用户反馈（左下角信封） ================= */
  var fbFab = null, fbPanel = null;
  function buildFeedback() {
    if (fbFab) return;
    fbFab = el("button", "fb-fab");
    fbFab.type = "button";
    fbFab.setAttribute("aria-label", "提交建议");
    fbFab.title = "给站长提个建议";
    fbFab.innerHTML =
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
      '<path d="M4 8.5 12 3l8 5.5"/><rect x="3" y="8.5" width="18" height="11.5" rx="2.2"/><path d="M3.6 9.4 12 15l8.4-5.6"/></svg>';
    document.body.appendChild(fbFab);
    fbPanel = el("div", "fb-panel");
    fbPanel.hidden = true;
    fbPanel.innerHTML =
      '<div class="fb-head">✉️ 给站长提个建议 <button type="button" class="fb-x" aria-label="关闭">×</button></div>' +
      '<div class="fb-cats">' +
      '<button type="button" class="fb-cat on" data-cat="knowledge">📚 建议收录</button>' +
      '<button type="button" class="fb-cat" data-cat="improve">🛠 改进建议</button>' +
      "</div>" +
      '<textarea class="fb-text" rows="4" maxlength="1000" placeholder="想看什么知识点？或哪里可以做得更好？（5-1000 字）"></textarea>' +
      '<div class="fb-mine-line"><button type="button" class="fb-mine">我的反馈记录</button><span class="fb-mine-empty"></span></div>' +
      '<div class="fb-err" hidden></div>' +
      '<button type="button" class="fb-send">发送给站长</button>' +
      '<div class="fb-mine-list" hidden></div>';
    document.body.appendChild(fbPanel);

    fbFab.addEventListener("click", function () {
      var open = fbPanel.hidden;
      fbPanel.hidden = !open;
      fbFab.classList.toggle("on", open);
      if (open) { fbPanel.__loadMine(); setTimeout(function () { var t = fbPanel.querySelector(".fb-text"); if (t && window.innerWidth >= 768) t.focus(); }, 120); }
    });
    fbPanel.querySelector(".fb-x").addEventListener("click", function () { fbPanel.hidden = true; fbFab.classList.remove("on"); });

    fbPanel.querySelectorAll(".fb-cat").forEach(function (c) {
      c.addEventListener("click", function () {
        fbPanel.querySelectorAll(".fb-cat").forEach(function (x) { x.classList.remove("on"); });
        c.classList.add("on");
      });
    });

    fbPanel.querySelector(".fb-send").addEventListener("click", function () {
      var btn = this;
      var onCat = fbPanel.querySelector(".fb-cat.on");
      var cat = onCat ? onCat.dataset.cat : "knowledge";
      var text = fbPanel.querySelector(".fb-text").value.trim();
      var err = fbPanel.querySelector(".fb-err");
      err.hidden = true;
      if (text.length < 5) { err.textContent = "再多写几个字吧（至少 5 字）"; err.hidden = false; return; }
      btn.disabled = true; btn.textContent = "发送中…";
      fetch("/api/feedback", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category: cat, content: text })
      }).then(function (r) { return r.json(); }).then(function (d) {
        btn.disabled = false; btn.textContent = "发送给站长";
        if (!d.ok) { err.textContent = d.err || "发送失败"; err.hidden = false; return; }
        fbPanel.querySelector(".fb-text").value = "";
        var ok = el("div", "fb-ok");
        ok.textContent = FAI.user ? "已送达站长 ✅ 在「我的反馈记录」里可跟踪状态" : "已送达站长 ✅ 登录后提交可跟踪采纳状态";
        fbPanel.insertBefore(ok, fbPanel.querySelector(".fb-mine-line"));
        setTimeout(function () { ok.remove(); }, 3200);
        fbPanel.__loadMine();
      }).catch(function () { btn.disabled = false; btn.textContent = "发送给站长"; err.textContent = "网络异常，稍后再试"; err.hidden = false; });
    });

    fbPanel.querySelector(".fb-mine").addEventListener("click", function () {
      var list = fbPanel.querySelector(".fb-mine-list");
      if (!list.hidden) { list.hidden = true; return; }
      fbPanel.__loadMine(function () { list.hidden = false; });
    });

    function loadMine(cb) {
      var list = fbPanel.querySelector(".fb-mine-list");
      var empty = fbPanel.querySelector(".fb-mine-empty");
      if (!FAI.user) { empty.textContent = "登录后可跟踪采纳状态"; list.hidden = true; list.innerHTML = ""; if (cb) cb(); return; }
      fetch("/api/feedback/mine").then(function (r) { return r.json(); }).then(function (d) {
        if (!d.ok) { empty.textContent = d.err || ""; list.hidden = true; if (cb) cb(); return; }
        var items = d.items || [];
        empty.textContent = items.length ? items.length + " 条" : "";
        if (!items.length) { list.innerHTML = '<div class="fb-none">还没有提交过反馈，写一条试试吧</div>'; if (cb) cb(); return; }
        list.innerHTML = items.map(function (f) {
          var catName = f.category === "improve" ? "🛠 改进" : "📚 收录";
          var st = f.merged ? '<span class="fb-st merged">已采纳 ✓</span>' : '<span class="fb-st pending">待处理</span>';
          return '<div class="fb-item"><div class="fb-item-top">' + catName + st + "</div>" +
            '<div class="fb-item-text">' + esc(f.content.slice(0, 80)) + "</div>" +
            '<div class="fb-item-date">' + esc(f.created_at.slice(0, 16)) + "</div></div>";
        }).join("");
        if (cb) cb();
      }).catch(function () { empty.textContent = ""; if (cb) cb(); });
    }
    fbPanel.__loadMine = loadMine;
  }
